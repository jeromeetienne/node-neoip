var url_builder_oload_t	= require('./url_builder_oload_t').url_builder_oload_t;
var assert		= {
...
test("test get/set/has/del consistency", function(){
	...
	var nested_uri	= new url_builder_oload_t()
...
test("build with dupuri", function(){
	...
	var result	= (new url_builder_oload_t(result)).to_string();
...
test('parse/build consistency', function(){
	...
	for(var i = 0; i < testvals.length; i ++){
		...
		var nested_uri	= new url_builder_oload_t(expected);
		var result	= nested_uri.to_string();
